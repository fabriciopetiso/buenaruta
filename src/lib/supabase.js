...
export const signUp = async (email, password, username, moto) => {
  ...
  // Update profile with username and moto info
  if (data.user) {
...
export const fetchRoutes = async (limit = 50) => {
  const { data, error } = await supabase
    ...
    .select(`
      *,
      profiles:user_id (id, username, moto_modelo, moto_cilindrada, moto_anio),
      route_likes (user_id),
      route_comments (id, user_id, text, created_at, profiles:user_id (username))
    `)
...
export const addComment = async (routeId, userId, text) => {
  const { data, error } = await supabase
    .from('route_comments')
    .insert({ route_id: routeId, user_id: userId, text })
...
export const fetchSavedRoutes = async (userId) => {
  const { data, error } = await supabase
    .from('saved_routes')
    .select(`*, routes (*, profiles:user_id (id, username, moto_modelo))`)
    .eq('user_id', userId)
...
export const fetchUserRoutes = async (userId) => {
  const { data, error } = await supabase
    ...
    .select(`
      *,
      profiles:user_id (id, username, moto_modelo),
      route_likes (user_id),
      route_comments (id)
    `)
    .eq('user_id', userId)
...
export const subscribeToLikes = (callback) => {
  return supabase
    ...
    .on('postgres_changes',
      { event: '*', schema: 'public', table: 'route_likes' },
      callback
    )
    .subscribe()
}
...
export const subscribeToComments = (callback) => {
  return supabase
    ...
    .on('postgres_changes',
      { event: '*', schema: 'public', table: 'route_comments' },
      callback
    )
    .subscribe()
}
